...
3. **Slash commands**: `/skill-name` means "the ccgs-skill-name skill". Installed set: start, brainstorm, design-system, create-architecture, create-epics, create-stories, sprint-plan, dev-story, code-review, story-done. If a referenced skill is NOT in this set, tell the user it isn't installed and offer to do the equivalent steps manually.
...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   See `refs/director-gates.md (bundled with this skill)` for the full check pattern.
...
- **`creative-director`** — gate **CD-PILLARS** (`refs/director-gates.md (bundled with this skill)`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`refs/director-gates.md (bundled with this skill)`)
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`refs/director-gates.md (bundled with this skill)`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`refs/director-gates.md (bundled with this skill)`).**
